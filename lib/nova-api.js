...
const DEVICE_TYPE_ANDROID_DEVICE = 1;
...
/**
 * Same request as listDevices(), but for ANDROID_DEVICE entries (phones/
 * tablets) instead of SPOT_DEVICE (Bluetooth trackers), and returns the raw
 * decoded protobuf objects unfiltered. Nova's response for phones/tablets is
 * currently undocumented territory for this adapter (listDevices() only
 * ever asks for SPOT_DEVICE, which comes back with `information: null` for
 * anything that isn't one) - this exists to find out what Google actually
 * sends back for the other device type before writing any real handling.
 *
 * @param {string} admToken - same scope as listDevices()
 */
async function listDevicesRaw(admToken, deviceType = DEVICE_TYPE_ANDROID_DEVICE) {
  const root = await loadProto();
  const DevicesListRequest = root.lookupType('googlefindmydevice.DevicesListRequest');
  const DevicesList = root.lookupType('googlefindmydevice.DevicesList');
  ...
  if (!response.ok) {
    const text = await response.text();
    throw new Error(`Nova ListDevices (raw) failed: HTTP ${response.status} - ${text.slice(0, 300)}`);
  }

  const buf = Buffer.from(await response.arrayBuffer());
  const decoded = DevicesList.decode(buf);
  return DevicesList.toObject(decoded, { longs: String, defaults: true, enums: String });
}

module.exports = { listDevices, executeLocateAction, listDevicesRaw };
