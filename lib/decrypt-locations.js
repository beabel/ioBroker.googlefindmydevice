...
function decryptEik(ownerKey, encryptedEik) {
  if (encryptedEik.length === 48) return decryptAesCbcNoPadding(ownerKey, encryptedEik);
  if (encryptedEik.length === 60) return decryptAesGcm(ownerKey, encryptedEik);
  throw new Error(`Unerwartete Laenge des verschluesselten Identity Keys: ${encryptedEik.length}`);
}
...
function retrieveIdentityKey(ownerKey, deviceRegistration) {
  ...
  if (!secrets || !secrets.encryptedIdentityKey) {
    throw new Error('Kein verschluesselter Identity Key fuer dieses Geraet vorhanden.');
  }
...
async function decryptLatestLocation(ownerKey, device) {
  ...
  if (!deviceRegistration) return null;
  ...
  if (!report || !report.encryptedLocation) return null;
