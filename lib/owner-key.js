...
function extractSharedKeyFromVaultKeys(vaultKeysJson) {
    ...
    if (!Array.isArray(entries) || entries.length === 0) {
        throw new Error('No "finder_hw" key found in the pasted result.');
    }
...
async function retrieveOwnerKey(spotToken, sharedKey) {
    ...
    if (!meta || !meta.encryptedOwnerKey) {
        throw new Error('Google did not return an encrypted owner key.');
    }
...
//
// Collapsed to a single line (via the .replace() below) so it shows up as
// one clean, fully copyable line in the ioBroker log instead of several -
// the log viewer otherwise makes it hard to tell where the snippet starts
// and ends.
const CONSOLE_SNIPPET = `(function(){
  window.mm = {
    setVaultSharedKeys: function(str, vaultKeys) {
      ...
      el.focus();
      el.select();
      console.log('Result captured - see the text field at the top of the page.');
    },
    closeView: function() {
      console.log('closeView() was called without providing data - something went wrong.');
    }
  };
  console.log('Listener installed. Now do whatever Google asks for on this page.');
})();`.replace(/\r?\n\s*/g, '');
