...
class McsClient {
  ...
  _connect() {
    ...
    this._socket = tls.connect({ host: MCS_HOST, port: MCS_PORT, servername: MCS_HOST }, () => {
      this.log.debug('MCS: TLS-Verbindung aufgebaut, sende Login...');
      this._login();
    });
    ...
    this._socket.on('error', err => {
      this.log.debug(`MCS-Verbindungsfehler: ${err.message}`);
    });
  ...
  _send(typeName, payload) {
    ...
    if (errMsg) throw new Error(`MCS ${typeName} ungueltig: ${errMsg}`);
  ...
  _handleFrame(tag, payload) {
    ...
    if (!typeName) {
      this.log.debug(`MCS: unbekanntes Tag ${tag} empfangen, ignoriere.`);
      return;
    }
    ...
    switch (typeName) {
      case 'LoginResponse':
        if (obj.error && obj.error.code) {
          this.log.warn(`MCS-Login fehlgeschlagen: ${obj.error.message || obj.error.code}`);
          return;
        }
        this._loggedIn = true;
        this.log.debug('MCS: erfolgreich angemeldet.');
      ...
      case 'HeartbeatAck':
        break;
      case 'Close':
        this.log.debug('MCS: Server hat die Verbindung geschlossen.');
        this._socket.destroy();
        break;
      case 'DataMessageStanza':
        this._handleDataMessage(obj).catch(err => this.log.debug(`MCS: Push-Nachricht konnte nicht verarbeitet werden: ${err.message}`));
        break;
      default:
        break;
    }
    ...
    this._heartbeatTimer = setInterval(() => {
      if (Date.now() - this._lastMessageTime > HEARTBEAT_INTERVAL_MS && this._loggedIn) {
        try {
          ...
          this.log.debug(`MCS: Heartbeat konnte nicht gesendet werden: ${err.message}`);
        }
      }
    }, HEARTBEAT_CHECK_MS);
  }
  ...
  async _handleDataMessage(msg) {
    ...
    try {
      ...
      this.log.debug('MCS: Push-Nutzlast war kein JSON, ignoriere.');
      return;
    }
  ...
  _sendSelectiveAck(persistentId) {
    try {
      ...
      this.log.debug(`MCS: Selective-Ack konnte nicht gesendet werden: ${err.message}`);
    }
  }
}
