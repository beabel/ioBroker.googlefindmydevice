...
async function gcmRegister(androidId, securityToken) {
  ...
  if (!response.ok || text.includes('Error')) {
    throw new Error(`GCM register3 fehlgeschlagen: ${text.slice(0, 200)}`);
  }
...
async function fcmInstall(clientSig) {
  ...
  if (!response.ok) {
    throw new Error(`FCM install fehlgeschlagen: HTTP ${response.status} - ${(await response.text()).slice(0, 200)}`);
  }
...
async function fcmRegister(gcmToken, keys, installToken, clientSig) {
  ...
  if (!response.ok) {
    throw new Error(`FCM register fehlgeschlagen: HTTP ${response.status} - ${(await response.text()).slice(0, 200)}`);
  }

  const json = await response.json();
  if (!json.token) {
    throw new Error('FCM register: Antwort enthielt kein Token.');
  }
  return json.token;
}
