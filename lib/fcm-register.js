...
async function fcmInstall(clientSig) {
    ...
    const json = /** @type {{authToken: {token: string}}} */ (await response.json());
...
async function fcmRegister(gcmToken, keys, installToken, clientSig) {
    ...
    const json = /** @type {{token?: string}} */ (await response.json());
    if (!json.token) {
 ...
 * now" request.
 *
 * @param {{androidId: string, securityToken: string, clientSig?: string}} identity the GCM checkin identity to register FCM push for
 * @returns {Promise<{fcmToken: string, gcmAppId: string, ecdhPrivateKey: Buffer,
 *   ecdhPublicKey: Buffer, authSecret: Buffer}>} the FCM token and keys needed to decrypt pushed location updates
 */
